...
behavior_questions = [
    ...
    "I demonstrate adaptability in changing situations.",
    "I show initiative beyond my role.",
    "I reflect on feedback and grow.",
    "I lead or influence others positively.",
    "I align my actions with company values."
]
...
def candidate_journey():
    ...
    if step == 0:
        ...
        for q in behavior_questions:
            a = st.radio(q, opts, horizontal=True)
            total += score_map[a]
        avg = round((total / (len(behavior_questions) * 5)) * 100, 1)
        st.session_state.behavior_score = avg
        st.metric("Behavior Score", f"{avg}/100")
        st.button("Back", on_click=prev_step)
        st.button("Next", on_click=next_step)
        ...
        st.selectbox("Highlight Trait (Ref 1)", traits)
        st.text_input("Reference 2 Name")
        st.text_input("Reference 2 Email")
        st.selectbox("Highlight Trait (Ref 2)", traits)
        st.button("Back", on_click=prev_step)
        st.button("Next", on_click=next_step)
        ...
        st.selectbox("Trait to Validate (Backchannel)", ["Initiative", "Growth Mindset", "Integrity", "Adaptability"])
        st.text_area("Optional: What should they highlight?")
        st.button("Back", on_click=prev_step)
        st.button("Next", on_click=next_step)
